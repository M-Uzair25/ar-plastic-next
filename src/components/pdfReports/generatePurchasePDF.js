...
export function generatePurchasePDF(purchases) {
  ...
  const tableRows = purchases.map((purchase, index) => [
    ...
    `${purchase.bagQuantity || 0} Bag${purchase.kgQuantity ? `, ${purchase.kgQuantity} Kg` : ''}`,
    purchase.category,
  ...
  for (let i = 1; i <= pageCount; i++) {
    doc.setPage(i);
    doc.setFontSize(10);
    doc.text(`Generated on: ${format(new Date(), 'dd/MM/yyyy')}`, 15, doc.internal.pageSize.height - 7);
    doc.text(`Page ${i} of ${pageCount}`, doc.internal.pageSize.width - 35, doc.internal.pageSize.height - 7);
  }
